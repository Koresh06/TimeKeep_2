...
def test_approve_changes_status(day_off_pending):
    day_off_pending.approve()
    assert day_off_pending.status == DayOffStatus.APPROVED


def test_reject_changes_status(day_off_pending):
    day_off_pending.reject()
    assert day_off_pending.status == DayOffStatus.REJECTED


def test_approve_already_moderated_raises_exception(day_off_approved):
    with pytest.raises(DayOffAlreadyModeratedError):
        day_off_approved.approve()


def test_reject_already_moderated_raises_exception(day_off_rejected):
    with pytest.raises(DayOffAlreadyModeratedError):
        day_off_rejected.reject()


def test_format_overtimes_for_report(day_off_pending):
